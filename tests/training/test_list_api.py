...
date_3 = datetime.now(timezone.utc) + timedelta(days=2)
date_4 = datetime.now(timezone.utc) + timedelta(days=2)


def test_get_date_list(client, mongo):
    fill_cololections(mongo)
    date = date_3.date().isoformat()
    response = client.get("/api/v1/trainings/list?category=18U&date=" + date)
